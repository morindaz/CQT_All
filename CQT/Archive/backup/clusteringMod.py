...
def read_data():
    ...
    #特征筛选 过滤型
    data = SelectKBest(chi2, k=1000).fit_transform(data, label)
...
def dimension_reduction(data, index, method='tsne', label=None, plot=False):
    ...
    #tsne预处理
    # model = TSNE(n_components=100, perplexity=40, early_exaggeration=100, method='exact',
    #              learning_rate=100, n_iter=1000, random_state=250, verbose=2)
    # X = model.fit_transform(data)
    #用pca做预处理
    # modelPca = PCA(n_components=78)
    # data = modelPca.fit_transform(data)
    # 大多数情况下，用tsne 将高纬度数据用二维方式展示出来。不同方法采用不同的特征映射方法计算出
    # 不同的X，用fittransform方法进行标准化，这里是最小-最大规范化
    if method == 'tsne':
        model = TSNE(n_components=n_components, perplexity=25, early_exaggeration=10, method='exact',
                     learning_rate=100, n_iter=1000, random_state=250, verbose=2)
...
if __name__ == "__main__":
    ...
    X = dimension_reduction(data, index, method='isomap', label=label, plot=True)
