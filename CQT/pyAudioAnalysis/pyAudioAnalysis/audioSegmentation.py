...
#def speakerDiarization(fileName, numOfSpeakers, mtSize=2.0, mtStep=0.2, stWin=0.1, LDAdim=35, PLOT=False):
#def speakerDiarization(fileName, numOfSpeakers, mtSize=1.0, mtStep=0.1, stWin=0.05, LDAdim=35, PLOT=False): Optimal
def speakerDiarization(pts, fileName, numOfSpeakers, mtSize=1.0, mtStep=0.1, stWin=0.05, LDAdim=35, PLOT=False):
    '''
    ARGUMENTS:
    ...
    [Classifier1, MEAN1, STD1, classNames1, mtWin1, mtStep1, stWin1, stStep1, computeBEAT1] = aT.loadKNNModel(os.path.join("knnSpeakerAll"))
    [Classifier2, MEAN2, STD2, classNames2, mtWin2, mtStep2, stWin2, stStep2, computeBEAT2] = aT.loadKNNModel(os.path.join("knnSpeakerFemaleMale"))
    ...
    if PLOT:
        ...
        for i in range(len(pts)):
            plt.axvline(float(pts[i]*0.1),color="r")
        plt.show()
    return cls
